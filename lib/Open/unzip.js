...
module.exports = async function unzip(source,offset,_password, directoryVars) {
  var file = PullStream(),
      entry = Stream.PassThrough();

  var req = await source.stream(offset);
  req.pipe(file).on('error', function(e) {
    ...
    entry.vars.then(async function(vars) {
      ...
      var stream = await file.stream(eof);
